...
func (s *SpecService) CompilePolicy(ctx context.Context, req PolicyCompileRequest) (*PolicyCompileResponse, error) {
	...
	actionDSL, diagnostics := s.parseEnglishPolicy(req.English, req.PolicyID, req.Version, req.Metadata)
...
func (s *SpecService) parseEnglishPolicy(english, policyID, version string, metadata map[string]string) (ActionDSLPolicy, []Diagnostic) {
	var diagnostics []Diagnostic
	var rules []DSLRule
	var permissions []Permission
	
	// Normalize text
	text := strings.ToLower(strings.TrimSpace(english))
