...
Maps findings **F01–F39** from [full-repo-audit-2026-07-01.md](full-repo-audit-2026-07-01.md) to remediation waves, status, burn-down IDs, and CI proof. Established during **Wave 0** reconciliation (2026-07-01). Last verified against code: **2026-07-22** (Wave 9+ audit-gap remediation **DONE** in working tree — fail-closed DSSE default, runtime/SDK stubs closed, CI cost/honesty, workspace + Lean Extended, live ops wiring, P3 polish; Dependabot still deferred).
...
**North-star:** inventory exit 0 on gated push/schedule workflows (Wave 7 **60/60** @ `7d48b3d4`; Wave 8 **69** gated via honest smokes — **not** literal 67/67); **DSSE fail-closed by default** (`PF_ENFORCE_DSSE=0`/`false` opt-out only); live AWS/registry/revocation/edge paths dispatch-only and fail-closed; burn-down + planning docs match code. Wave 9+ (T1–T18) **closed** (T12 **ACCEPTED** by policy; Dependabot deferred).
...
**Historical green snapshot (tip `bae36f642`, 2026-07-18):** inventory exit **0 ×2** — **69** gated (push/schedule), **0** red after **PR #223** CI-local proofs. Wave 7 historical **60/60** @ `b8b78b94` remains the pre-revive baseline. Do **not** claim literal 67/67, and do not treat this snapshot as the current inventory.
...
**Note:** **PR #206 merged** at `7d48b3d4`; tip **`b8b78b94`** after **PR #207** (2026-07-16). Inventory **60/60** exit 0 ×2. Phase 3 hardening proof + Phase 4 sign-off: [archive/wave7-post-merge-runbook.md](archive/wave7-post-merge-runbook.md) Phase D/E. F23/F24 **DONE**.
...
| F08 | P0 | Broken edge-middleware example import | 5 | **DONE** | — | — | Stub removed in structural cleanup; see `docs/guides/demos.md` |
...
| F11 | P1 | Vacuous Jest gates | 1, 4 | **DONE** | TD-005–TD-011 | — | Ledger 23+ Jest tests; SDK 4 tests in `reusable-ci-go-node.yml` |
...
| 8 | Revive leftovers + Lean | F33 + re-gates | 69 gated exit 0; lean-offline-full; CI-local moto/mock proofs | **DONE** — see Wave 8 revive note below |
| 9–14 | Audit-gap remediation (T1–T18) | Deep-audit 2026-07-22 | See **Wave 9+** section | **DONE** (working tree 2026-07-22); Dependabot deferred |
| E | Engineering speed / scaffolding (non-Lean) | Launch, wiring, CI paths, composite actions | Docs-only &lt;5 min; warm ledger-up &lt;60s; budgets + schedule smoke | **DONE** (working tree 2026-07-22) — see **Wave E** |
...
**Wave 7 inventory gate:** **DONE** — inventory exit **0** twice on `main` @ `7d48b3d4` (**60/60** gated green); tip `b8b78b94` after #207. Phase 3+4: [archive/wave7-post-merge-runbook.md](archive/wave7-post-merge-runbook.md).
...
## Wave 9+ — Audit-gap remediation (2026-07-22)

Source: deep audit canvas @ tip `d1db030ea` (findings **T1–T18**). Builds on Waves 0–8 (**DONE**). All waves **implemented in working tree** (2026-07-22). Dependabot conflict program remains **deferred** (not a T-finding blocker).
...
| 11.1–11.3 | Multiarch / Criterion / paper-conformance cost | T10, T11 (+ paper cost) | **DONE** |
| 11.4–11.5 | Soft-fail harden + Windows smoke + WSL-first | T14, T16 | **DONE** (honesty **50** justified) |
...
|----|-----|-------------------|------|--------|---------------|
| T1 | P0 | DSSE fail-open by default | 9.1 | **DONE** | Go/Rust/TS/dsse-ts/ledger/sidecar: unset → enforce; opt-out `PF_ENFORCE_DSSE=0`/`false`; trust root required when enforcing |
...
| T13 | P2 | MicroInterp Extended.Event adapter unfinished | 12.2 | **DONE** | `ActionDSL.Extended` + `Runtime/ExtendedAdapter.lean` lake roots; ENFORCED sorry scan; **mathlib-backed** — built on `lean-offline-full` (`lake build ActionDSL.Extended Runtime.ExtendedAdapter`), not PR smoke |
| T14 | P2 | Soft-fail / honesty cluster (~62 → ~51) | 11.4 | **DONE** | Soft-fail harden set; `audit_ci_honesty.py` exit 0 — **50** justified, 0 unjustified (OpenHands eval soft-ignore removed) |
...
### Wave 13 — Live ops (locked include)

| Path | Workflow | Gated floor | Live path | Status |
|------|----------|-------------|-----------|--------|
| Cross-region DR | `dr-cross.yaml` | moto | `workflow_dispatch` `mode=live` | **DONE** — fail-closed preflight (`scripts/dr/live_dr_preflight.py`) |
| Registry publish | `publish-updates.yaml` | dry-run + mock | `dry_run=false` | **DONE** — `live_registry: true` artifact when secrets present |
| Revocation sync | `revocation-sync.yaml` | dry-run mock | `mode=live` | **DONE** — `scripts/revocation/live_registry_sync.py` |
| Edge load | `edge-load.yaml` | smoke mock | `mode=full` | **DONE** — fail-closed without `EDGE_REGION_URLS` |
...
| Check | Result |
|-------|--------|
| `python scripts/audit_ci_honesty.py` | exit **0** — **50** justified, 0 unjustified |
| DSSE default (ports) | unset → enforce across Go/Rust/TS/ledger/sidecar |
...
| Item | Verified / fixed |
|------|------------------|
| Tracker ↔ burn-down ↔ inventory | TD-001/002/006/007/010/011 **DONE**; inventory Redis/LabelerGen claims refreshed |
...
**Closed this pass (2026-07-22):** `policykernel/compiler` DFA unit tests — last-condition transitioned via synthetic `condition_satisfied` hop so `Evaluate`/`EvaluatePath` never reached accepting states; compiler now accepts on the final real condition event; `go test ./compiler/` green.

---

## Wave E (Engineering) — speed and scaffolding (2026-07-22)
...
### North-star before → after (estimates)

| Metric | Before (pre-E) | After (E1–E5 landed) | Status |
|--------|----------------|----------------------|--------|
| Docs-only `main` push gated wall-clock | Full `ci.yml` language matrix (Rust 120m budget + Go/Node multi-`npm ci` + extended) | Path-conditioned: docs/figs skip Rust/Go-Node/extended/Lean slices; target **&lt;5 min** gated | **Landed** (E3.1) — measure per budget doc |
| Sidecar-only PR | Broad Go/Node + console install tax; full-ish Rust workspace pressure | Impacted Rust crates + nextest; Go/Node parallel slices; console path-scoped | **Landed** (E3.2–E3.3) |
| Warm `make ledger-up` → GraphQL healthy | `demo-up --build` + fixed `sleep 30` / heavy default compose | `ledger-up` + `--wait`, no `--build` on warm path; target **&lt;60s** | **Landed** (E1) — local measure |
| Compose / wiring feedback | Misaligned ports; discover-after-boot | `check_wiring.py` + `compose-smoke`; pf-env schema | **Landed** (E1–E2) |
...
| Contributor setup | K8s implied; duplicated Actions setup; 25 go.mod islands | `local-workflows.md`; composite actions; `go-work-init`; path-aware `install-dev`; Kind path-gated | **Landed** (E4) — reusable Go/Node composites adopted |
...
| E3.2–E3.3 | Split reusable Rust / Go-Node; npm workspaces; impacted installs | **DONE** (working tree) |
| E3.4–E3.5 | Schedule/path-filter burners; inventory + ci-reference honesty | **DONE** (working tree) |
...
### Spot-check — E1–E4 artifacts present
...
| `reusable-ci-go-node.yml` parallel go-cli / ledger-node / … | Yes |
...
| Item | Notes |
|------|-------|
| `reusable-ci-go-node.yml` adopt `setup-go-cli` / `setup-node-workspace` | **CLOSED** (2026-07-22) — go-cli, ledger-node, sdk-node, console use composites; pcs-spectral keeps plain `setup-node` (global spectral only) |
| First green `engineering-budget-smoke` on `main` | Schedule/dispatch-only; inventory will treat it as gated once present — dispatch after merge for a green last-run |
