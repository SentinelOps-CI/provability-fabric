...
Triage and historical notes for `main`. **Live counts live in the inventory + tracker — not in the archived tables below.**
...
## Live posture (Wave 8 — authoritative)

| Metric | Value | Source |
|--------|------:|--------|
| Gated (push/schedule on `main`) | **69** | [ci-inventory-latest.md](../ci-inventory-latest.md) (2026-07-18) |
| Inventory exit | **0** (×2 claimed) | [remediation-tracker.md](../remediation-tracker.md), [evidence-program-closure.md](../../roadmap/evidence-program-closure.md) |
| Wave 7 historical gate | **60/60** @ `7d48b3d4` / tip `b8b78b94` | Pre–Wave 8 revive baseline |
...
## Archived baseline (2026-07-03 — historical only)
...
**Phase 0–1 local prep (2026-07-03, archived):** Placeholder gate, CI honesty gate (`ci.yml`), replay contract, compose smoke, sidecar `integration_tests`, DSSE cross-lang, `retrieval-gateway.yml` — all later proved on `main` (Waves 1–8).

## Workflow cluster remediation checklist (Wave 7–8 — closed)

Clusters below were the Wave 7 greening program. Status reflects **post–Wave 8** reality. Forward CI cost/honesty work is Wave 11+ (not a re-open of this matrix).
...
| **Bench** | `bench-nightly-criterion.yaml`, `performance-gate.yaml`, `bench-swebench-smoke.yaml` | Wave 1 (F23) | **Green (gated)** | Criterion green on `main`; stress honesty → Wave 10.4 |
...
| **Core CI** | `ci.yml`, `proto-compat.yaml`, `actionlint.yml` | Wave 0–1 | **Green** | Includes `ci-honesty` gate |
| **Remaining** | docker, marketplace, DR, automation | Wave 7–8 | **Green (gated)** with CI-local proofs | Live AWS/registry still secret-gated (Wave 13) |

---

## Historical triage notes (2026-06-15 / PR #118 era)

The sections below are **archived narrative** from the Evidence program CI hardening era. Per-workflow last-run URLs and “13/68” / “8/67” figures are **not current**. For live status, re-run inventory or open [ci-inventory-latest.md](../ci-inventory-latest.md).
...
## Workflow inventory (Phase 0 baseline — archived 2026-06-15)

**Archived.** Full inventory of `.github/workflows/*` on `main` as of 2026-06-15. Last-run status from that date is stale. **Live:** [ci-inventory-latest.md](../ci-inventory-latest.md) — **69** gated, inventory exit **0** (Wave 8). Do not cite the per-row “Failure” URLs below as current.
...
### Inventory stats (archived 2026-06-17 — not live)
...
Historical blockers (CLA hosted API, org secrets, Kind/AWS infra, in-repo fixes) were triaged through Waves 1–8. Remaining forward items are **live-secret** AWS DR / registry publish / revocation (Wave 13) and CI cost/honesty (Wave 11) — not a return to the 13/68 red matrix.
