...
- **Trust chain (F01–F02, F17, F25):** Cross-language DSSE verify (Go/Rust/TS); **fail-closed by default** (unset `PF_ENFORCE_DSSE` enforces; opt out only with `0`/`false`); deny-by-default `PF_ENABLED_TOOLS`; evidence hash enforcement.
